...
export default function App() {
  ...
  // Dynamic SEO Page Title & Description (Targeting Construction LLP)
  useEffect(() => {
    try {
      if (activeView === 'bookings') {
        ...
        if (descMeta) {
          descMeta.setAttribute('content', 'View and manage your scheduled site visits and structural consultations with Panchratna Constructions LLP Ranchi.');
        }
      } else {
  ...
  // Dynamic Open Status Badge logic — Mon–Sat, 10:30 AM to 8:00 PM IST
  useEffect(() => {
    const checkStatus = () => {
      try {
        const now = new Date();
        ...
        const decimalTime = hour + (now.getMinutes() / 60);
        setIsOpenNow(day !== 0 && decimalTime >= 10.5 && decimalTime < 20.0);
      }
    };
    ...
    <div className="min-h-screen bg-[#FDFBF7] text-[#1E293B] selection:bg-[#115E59]/20 selection:text-[#115E59] font-sans relative z-0">
      ...
      {showIntro && (
        <div className="intro-overlay">
          <img src="/logo.png" alt="Panchratna Constructions LLP Logo" className="intro-logo" />
        </div>
      )}

      <div className="bg-watermark"></div>
      
      <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b border-[#EAE5DC] shadow-sm relative">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-8 text-sm font-semibold tracking-wide">
            <button onClick={() => { setActiveView('main'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`transition-colors cursor-pointer bg-transparent border-0 font-semibold tracking-wide p-0 ${activeView === 'main' ? 'text-[#115E59]' : 'text-[#5A6561] hover:text-[#115E59]'}`}>Home</button>
            <a href="#about" onClick={(e) => smoothScroll(e, 'about')} className="text-[#5A6561] hover:text-[#115E59] transition-colors">Legacy</a>
            <a href="#skyline-portfolio" onClick={(e) => smoothScroll(e, 'skyline-portfolio')} className="text-[#5A6561] hover:text-[#115E59] transition-colors">Portfolio</a>
            <button onClick={() => { setActiveView('bookings'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`transition-colors cursor-pointer bg-transparent border-0 font-semibold tracking-wide p-0 ${activeView === 'bookings' ? 'text-[#115E59]' : 'text-[#5A6561] hover:text-[#115E59]'}`}>My Site Visits</button>
          ...
          <div className="hidden lg:flex items-center gap-4">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-2xs font-extrabold tracking-widest border uppercase transition-colors ${isOpenNow ? 'bg-emerald-50 border-emerald-200 text-emerald-700 animate-pulse' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
            ...
            <a href="tel:9263002626" className="py-2 px-4.5 bg-[#115E59] text-white font-bold text-xs rounded-lg shadow-md hover:bg-[#0D4F4A] transition-all uppercase tracking-wider">Enquire Now</a>
          </div>

          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 text-[#5A6561] hover:text-[#115E59] transition-colors">
            {mobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
        </div>

        {mobileMenuOpen && (
          <div className="lg:hidden absolute top-20 left-0 w-full bg-white/95 backdrop-blur-xl border-b border-[#EAE5DC] p-6 flex flex-col gap-5 animate-fade-in shadow-md">
            <button onClick={() => { setActiveView('main'); setMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`text-left text-base font-bold cursor-pointer bg-transparent border-0 p-0 ${activeView === 'main' ? 'text-[#115E59]' : 'text-[#1A2421]'}`}>Home</button>
            <a href="#about" onClick={(e) => smoothScroll(e, 'about')} className="text-base font-semibold text-[#1A2421]">Legacy</a>
            <a href="#skyline-portfolio" onClick={(e) => smoothScroll(e, 'skyline-portfolio')} className="text-base font-semibold text-[#1A2421]">Portfolio</a>
            <button onClick={() => { setActiveView('bookings'); setMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`text-left text-base font-semibold cursor-pointer bg-transparent border-0 p-0 ${activeView === 'bookings' ? 'text-[#115E59]' : 'text-[#1A2421]'}`}>My Site Visits</button>
            ...
            <div className="border-t border-[#EAE5DC] pt-4 flex flex-col gap-4">
              <a href="tel:9263002626" className="w-full text-center py-3 bg-[#115E59] text-white font-bold rounded-xl shadow-md uppercase tracking-wider text-xs">Call Office — 92630 02626</a>
            </div>
          </div>
        )}
      </header>
      ...
      {activeView === 'bookings' ? (
        ...
        <>
          <section className="relative overflow-hidden pt-20 pb-24 md:pt-28 md:pb-32 bg-gradient-to-b from-[#FDFBF7]/90 via-[#F9F6F0]/90 to-[#FDFBF7]/90 z-10">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
              <div className="max-w-4xl">
                ...
                <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-[#1E293B] tracking-tight leading-[1.08] mb-8 uppercase">
                  Built on <span className="text-gradient-neon">Trust.</span> <br />
                ...
                <p className="text-base sm:text-lg md:text-xl text-[#64748B] leading-relaxed max-w-3xl mb-12">
                  Pioneers in high-rise structural engineering and luxury residential landmarks. From Altius to Heritage, we redefine Ranchi’s urban footprint through JHARERA approved excellence.
                </p>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                  <a href="#skyline-portfolio" onClick={(e) => smoothScroll(e, 'skyline-portfolio')} className="btn-neon-emerald py-4 px-8 rounded-xl flex items-center justify-center gap-2 text-sm uppercase tracking-wider font-bold">
                    <Building className="w-5 h-5" /> Explore Projects
                  </a>
                  <a href="https://wa.me/919263002626" target="_blank" className="btn-neon-cyan-outline py-4 px-8 rounded-xl flex items-center justify-center gap-2 text-sm uppercase tracking-wider font-bold">
                    <PhoneCall className="w-5 h-5" /> Direct Inquiry
                  </a>
                </div>
              </div>
            </div>
          ...
          <section id="about" className="py-16 bg-white/90 border-b border-[#EAE5DC] relative z-10">
            <div className="max-w-4xl mx-auto px-6 text-center space-y-4">
              ...
              <p className="text-sm md:text-base text-[#64748B] leading-relaxed">
                Panchratna Constructions LLP is Ranchi's premier civil engineering firm, dedicated to creating sustainable urban ecosystems. Located in the heart of the city at Sarjana Chowk, we specialize in IGBC-certified green buildings and ultra-luxury high-rises.
              </p>
            </div>
          </section>
            ...
            <div className="max-w-7xl mx-auto px-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                <div className="flex gap-4.5">
                  <div className="w-12 h-12 rounded-xl bg-white border border-[#EAE5DC] flex items-center justify-center text-[#115E59] shadow-sm"><ShieldCheck className="w-6 h-6" /></div>
                  <div className="space-y-1">
                    <h3 className="font-extrabold text-[#1E293B] text-lg uppercase tracking-tight">JHARERA &amp; IGBC Certified</h3>
                    <p className="text-xs text-[#64748B]">100% RERA compliant and engineered to Indian Green Building Council standards.</p>
                  </div>
                </div>
                <div className="flex gap-4.5">
                  <div className="w-12 h-12 rounded-xl bg-white border border-[#EAE5DC] flex items-center justify-center text-[#0F766E] shadow-sm"><Building2 className="w-6 h-6" /></div>
                  <div className="space-y-1">
                    <h3 className="font-extrabold text-[#1E293B] text-lg uppercase tracking-tight">30+ Delivered Landmarks</h3>
                    <p className="text-xs text-[#64748B]">Proven track record of delivering iconic residential and commercial complexes across Ranchi.</p>
                  </div>
                </div>
                <div className="flex gap-4.5">
                  <div className="w-12 h-12 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-700 shadow-sm"><Star className="w-6 h-6" /></div>
                  <div className="space-y-1">
                    <h3 className="font-extrabold text-[#1E293B] text-lg uppercase tracking-tight">4.9★ Structural Rating</h3>
                    <p className="text-xs text-[#64748B]">Recognized for superior construction quality and seismic-resistant architectural designs.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* === MIDPOINT ANCHOR: STRUCTURAL PORTFOLIO === */}
          <ProjectGallery />
          ...
          <section id="projects" className="py-24 bg-[#FDFBF7]/90 relative z-10">
            <div className="max-w-7xl mx-auto px-6">
              <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold uppercase tracking-wider">Project Consultation &amp; Site Visits</div>
                <h2 className="text-3xl md:text-4xl font-extrabold text-[#1E293B] tracking-tight uppercase">Schedule Your Site Visit</h2>
                <p className="text-sm md:text-base text-[#64748B] leading-relaxed">Experience the quality of our construction firsthand. Book a slot for a guided tour of our active project sites.</p>
              </div>
              <div className="relative"><BookingCalendar /></div>
              <div className="mt-8 max-w-4xl mx-auto bg-gradient-to-r from-emerald-50/80 to-teal-50/80 border border-emerald-200/60 rounded-2xl p-5 shadow-sm backdrop-blur-md flex flex-col sm:flex-row items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center text-white shrink-0 shadow-sm"><HardHat className="w-6 h-6" /></div>
                <div className="space-y-1">
                  <h4 className="text-xs font-extrabold uppercase tracking-widest text-[#115E59]">Site Visit Verification</h4>
                  <p className="text-xs text-slate-600 font-medium">Need to verify your spot? Go to the <button onClick={() => { setActiveView('bookings'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-[#115E59] font-extrabold underline cursor-pointer bg-transparent border-0 p-0 inline uppercase">My Site Visits</button> tab to lookup your pass by phone number.</p>
                </div>
              </div>
            </div>
          </section>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div className="lg:col-span-5 space-y-8">
                  <div className="space-y-4">
                    <h2 className="text-3xl font-extrabold text-[#1E293B] tracking-tight uppercase">Head Office &amp; <br />Contact Hub</h2>
                    <p className="text-sm text-[#64748B]">Visit our headquarters at Panchratna Galleria, Sarjana Chowk, Ranchi. Our engineering consultants are available during office hours.</p>
                  </div>
                  <div className="bg-white border border-[#EAE5DC] rounded-2xl p-5 space-y-4 shadow-md shadow-[#EFEAE2]">
                    <h3 className="font-semibold text-[#1E293B] text-sm uppercase tracking-widest border-b border-[#EAE5DC] pb-3 flex items-center gap-2"><Clock className="w-4 h-4 text-[#115E59]" />Office Timings</h3>
                    <div className="space-y-2.5 text-xs font-bold uppercase">
                      <div className="flex justify-between"><span>Mon — Sat</span><span>10:30 AM — 08:00 PM</span></div>
                      <div className="flex justify-between text-slate-400"><span>Sunday</span><span>Closed</span></div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white border border-[#EAE5DC] rounded-2xl shadow-md"><span className="block text-3xs font-extrabold text-[#64748B] uppercase tracking-wider mb-2">Direct Hotline</span><a href="tel:9263002626" className="text-xs font-bold text-[#1E293B]">92630 02626</a></div>
                    <div className="p-4 bg-white border border-[#EAE5DC] rounded-2xl shadow-md"><span className="block text-3xs font-extrabold text-[#64748B] uppercase tracking-wider mb-2">Email Us</span><a href="mailto:info@panchratnaconstructions.com" className="text-xs font-bold text-[#1E293B] break-all">info@panchratnaconstructions.com</a></div>
                  </div>
                </div>
                <div className="lg:col-span-7 flex flex-col space-y-6">
                  <div className="bg-white border border-[#EAE5DC] rounded-2xl overflow-hidden shadow-md flex-1 min-h-[340px]">
                    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3663.957545929668!2d85.322448!3d23.3679259!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x39f4e13d9630739b%3A0xc3f95e54d7f57a0c!2sPanchratna%20Galleria!5e0!3m2!1sen!2sin!4v1716200000000!5m2!1sen!2sin" width="100%" height="100%" style={{ border: 0, minHeight: '340px' }} allowFullScreen="" loading="lazy" title="Panchratna Galleria Location"></iframe>
                  </div>
                  <div className="flex gap-4 items-center bg-white p-4 border border-[#EAE5DC] rounded-xl shadow-md">
                    <div className="w-10 h-10 rounded-lg bg-teal-50 border border-teal-200 flex items-center justify-center text-[#0F766E] shrink-0"><Building className="w-5 h-5" /></div>
                    <div>
                      <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">Office Address</h4>
                      <p className="text-2xs text-[#64748B] mt-0.5 uppercase font-bold">311, Panchratna Galleria, Sarjana Chowk, Main Road, Ranchi, Jharkhand 834001</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <footer className="bg-white/90 border-t border-[#EAE5DC] py-12 relative z-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 text-2xs text-[#64748B] font-bold uppercase tracking-widest">
          <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-[#115E59]" /><span>&copy; 2026 Panchratna Constructions LLP. All rights reserved.</span></div>
          <div className="flex gap-6"><a href="#contact" onClick={(e) => smoothScroll(e, 'contact')} className="hover:text-[#115E59]">Contact Us</a></div>
        </div>
      </footer>
    </div>
  );
